...
use crate::circuit;
use crate::frontend;
use crate::frontend::api::BasicAPI;
...
impl<T: Clone + TensorType> Tensor<T> {
    ...
    /// ```
    /// use expander_compiler::tensor::{Tensor, TensorError};
    /// use expander_compiler::fieldutils::IntegerRep;
    /// let mut a = Tensor::<IntegerRep>::new(Some(&[1, 4]), &[2]).unwrap();
...
impl Tensor<Variable> {
    pub fn add_constraint<C: circuit::config::Config>(
        self,
        rhs: Self,
        builder: &mut frontend::API<C>,
    ) -> Result<Tensor<Variable>, TensorError> {
        let broadcasted_shape = get_broadcasted_shape(self.dims(), rhs.dims()).unwrap();
        let lhs = self.expand(&broadcasted_shape).unwrap();
        let rhs = rhs.expand(&broadcasted_shape).unwrap();
        let res = {
...
/// ```
/// use expander_compiler::tensor::get_broadcasted_shape;
/// let a = vec![2, 3];
/// let b = vec![2, 3];
